...
class Difference(Method):
    ...
    def apply(self, to_be_inspected, template_id=""):
        ...
        template_mod = cv.blur(template,(5,5))
        to_be_inspected_mod = cv.blur(to_be_inspected,(5,5))
        # Creating kernel
        kernel = np.ones((12, 12), np.uint8)
        # Using cv2.erode() method 
        template_mod = cv.erode(template_mod, kernel) 
        to_be_inspected_mod = cv.erode(to_be_inspected_mod, kernel)
        ...
        if self.method == "complex":
            ...
            bitwiseXor = cv.bitwise_xor(template_mod, to_be_inspected_mod)
            bitwiseXor[bitwiseXor >= self.simple_method_thresh] = 255
            bitwiseXor[bitwiseXor < self.simple_method_thresh] = 0
            # Region of Interest
            [row, col] = bitwiseXor.shape
            R_int = np.zeros((row,col), dtype = bitwiseXor.dtype)
            R_int_aux = np.ones((300,150)) * 255  #segundo es eje x
            
            [row_R_int, col_R_int] = R_int.shape
            [row_R_int_aux, col_R_int_aux] = R_int_aux.shape

            init_row = 500
            init_col = 460
            R_int[init_row:init_row + row_R_int_aux, init_col:init_col + col_R_int_aux] = R_int_aux
            bitwiseXor = cv.bitwise_and(bitwiseXor, R_int)
                ...
                if area > self.area_threshold:
                    boxes.append(
                        (
                            ...
                            w + 2 * self.shift,
                            h + 2 * self.shift,
                            #str(x) + ", " + str(y) + ", " + str(w) + ", " + str(h) + ", "+ str(round(cv.contourArea(c) * 0.0002688,2)) + " mm2"
                            str(round(cv.contourArea(c) * 0.0002688,2)) + " mm2",

                        )
                    )
            
            if len(boxes) > 0:
                to_be_inspected = cv.cvtColor(to_be_inspected, cv.COLOR_GRAY2BGR)
                #if len(to_be_inspected.shape) == 1 else to_be_inspected
                for box in boxes:
                    x, y, w, h, area_def = box
                    cv.rectangle(
                    ...
                    cv.putText(
                        to_be_inspected, area_def, (x-40, y-30), font,2,
                        tuple(self.bbox_color) , 3
                    )
                    cv.putText(
                        to_be_inspected, "91 %", (x-40, y-85), font,2,
                        tuple(self.bbox_color) , 3
                    )
                return (
                    "Defected",
